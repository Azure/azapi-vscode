...
export class ClientHandler {
  ...
  private getInitializationOptions(commandPrefix: string) {
    const rootModulePaths = config('azurerm-restapi-lsp').get<string[]>('rootModules', []);
    const terraformExecPath = config('azurerm-restapi-lsp').get<string>('terraformExecPath', '');
    const terraformExecTimeout = config('azurerm-restapi-lsp').get<string>('terraformExecTimeout', '');
    const terraformLogFilePath = config('azurerm-restapi-lsp').get<string>('terraformLogFilePath', '');
    const excludeModulePaths = config('azurerm-restapi-lsp').get<string[]>('excludeRootModules', []);
    const ignoreDirectoryNames = config('azurerm-restapi-lsp').get<string[]>('ignoreDirectoryNames', []);
